...
enum { SchemeNorm, SchemeSel, SchemeUns, SchemeUnf, SchemeHid, SchemeInv }; /* color schemes */
...
struct Monitor {
	...
	int gappxMin;            /* minimum gaps between windows */
...
{
	/* changed so that the min gap size is set by gappx */
	if ((arg->i == 0) || (selmon->gappx + arg->i < gappx))
		selmon->gappx = gappxMin;
	else
		selmon->gappx += arg->i;
	arrange(selmon);
}
